...
the directory containing the output of VOISE (CVDseeds.txt and
...
-------------------------------------------------------------------------------
kNN-enhance algorithm (k Nearest Neighbour) modifed from:
Caiyan Jia et al.
“Node Attribute-enhanced Community Detection in Complex Networks”.
Nature, Scientific Reports 7.1 (2017).
...
def extract_neighbours(filename):
    ...
    from VOISE output (CVDneighbours.txt)
    """
    d = {}
    n = 0
...
with Timer() as total_time:
    with Timer() as t:
        ...
        seed_info = np.loadtxt(root + "CVDseeds.txt", skiprows=3,comments='*')[:, 3:]
