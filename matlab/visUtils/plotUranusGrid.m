...
%
% $Id: plotUranusGrid.m,v 1.4 2015/09/15 18:30:34 patrick Exp $
%
% Copyright (c) 2009 
...
[sslat,sslon,selat,selon,CML,psi,sedistAU,AU_km] = computeUranusAxis(epoch);
...
if length(p)==2,
  ...
  [a,b,e,f] = getPlanetGeometry('Uranus');
...
if 0,
  ...
  drawUranusGrid(epoch,CML,psi,orientat,dlat,dlon,semiMaj_km,ecc);
end
toc
...
fprintf(1,'Earth-Uranus distance = %.4f AU (1 AU = %.3f km)\n',sedistAU,AU_km);

% Get difference between (right-handed) subearth and subsolar longitudes.
% Although these two quantities will vary on time scale of Uranus rotation
