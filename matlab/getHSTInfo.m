...
%
% $Id: getHSTInfo.m,v 1.6 2015/09/14 17:47:46 patrick Exp $
%
...
if ~isempty(TELESCOP) && strcmp(TELESCOP,'HST'),
  ...
  HST.TARGNAME = getFitsKeywordsValue(iFile,{'TARGNAME'});
	% right ascension and declination of the target (deg) (J2000)
  [HST.RA_TARG,HST.DEC_TARG] = getFitsKeywordsValue(iFile,{'RA_TARG','DEC_TARG'});
	% UT date and time of start of first exposure
  [HST.TDATEOBS,HST.TTIMEOBS] = getFitsKeywordsValue(iFile,{'TDATEOBS','TTIMEOBS'});
	% exposure duration (seconds)
  HST.EXPTIME  = getFitsKeywordsValue(iFile,{'EXPTIME'});
	% total exposure time (seconds)
	HST.TEXPTIME = getFitsKeywordsValue(iFile,{'TEXPTIME'});
  if isempty(HST.TDATEOBS) & isempty(HST.TTIMEOBS),
    [HST.TDATEOBS,HST.TTIMEOBS] = getFitsKeywordsValue(iFile,{'DATE-OBS','TIME-OBS'});
  end
  ...
  % from pixel coordinate to intermediate world coordinate
	[HST.CD1_1,HST.CD1_2] = getFitsKeywordsValue(iFile,{'CD1_1','CD1_2'});
	[HST.CD2_1,HST.CD2_2] = getFitsKeywordsValue(iFile,{'CD2_1','CD2_2'});
  ...
  [HST.RA_APER,HST.DEC_APER] = getFitsKeywordsValue(iFile,{'RA_APER','DEC_APER'});
  ...
  HST.ORIENTAT = getFitsKeywordsValue(iFile,{'ORIENTAT'});
